...
/// UILabel subclass that can display curved text.
/// Adapted from https://stackoverflow.com/a/42745528/3918865
class CurvedLabel: UILabel {
    ...
    func centreArcPerpendicular() {
        ...
        for i in 0 ..< l {
            arcs += [chordToArc(characters[i].size(withFont: self.font).width, radius: radius)]
            totalArc += arcs[i]
        }
        ...
        let slantCorrection = clockwise ? -π/2 : π/2
    ...
    func centre(text str: String, context: CGContext, radius r:CGFloat, angle theta: CGFloat, slantAngle: CGFloat) {
        ...
        let attributes: [NSAttributedStringKey : Any] = [NSAttributedStringKey.foregroundColor : self.textColor,
        ...
        let offset = str.size(withFont: self.font)
    ...
    func getRadiusForLabel() -> CGFloat {
        ...
        let heightOfFont = self.text?.size(withFont: self.font).height ?? 0
